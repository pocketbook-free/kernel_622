...
int set_wakeup_from_rtc(int wakeup_status)
{
	printk("wakeup_status : %d\n", wakeup_status);
	rtc_can_wakeup = wakeup_status;
	printk("rtc_can_wakeup : %d\n", rtc_can_wakeup);
	return 0;
}
...
static int mxc_rtc_probe(struct platform_device *pdev)
{
  ...
  hw_set_wakeup_from_rtc(set_wakeup_from_rtc);
...
{
	struct rtc_drv_data *pdata = platform_get_drvdata(pdev);
	printk("rtc_can_wakeup is %d\n", rtc_can_wakeup);

	if ((rtc_can_wakeup == 1) && device_may_wakeup(&pdev->dev)) {
