...
#define SD_NAME  "mxsdhci.2"
#define INAND_NAME "mxsdhci.0"
...
static int sdhci_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	hw_get_deepsuspend(&sleep_state);

	dev = &( *pdev).dev;
	if (dev->init_name)
	{
		device_name = dev->init_name;
	}
	else
	{
		device_name = kobject_name(&dev->kobj);
	}
	
	if (sleep_state == PM_SUSPEND_STANDBY)
	{
		if (!strcmp(device_name,INAND_NAME) || !strcmp(device_name,SD_NAME) )
		{
			return 0;
		}
	}
	else if (sleep_state == PM_SUSPEND_MEM)
	{
		if (!strcmp(device_name,INAND_NAME) )
		{
			return 0;
		}
	}
  	...
  	enable_irq_wake(gpio_to_irq(27));	
...
static int sdhci_resume(struct platform_device *pdev)
{
	...
	else if (sleep_state == PM_SUSPEND_MEM)
	{
  	...
  	disable_irq_wake(gpio_to_irq(27));
