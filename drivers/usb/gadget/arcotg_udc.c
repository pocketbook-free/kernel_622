...
#ifdef CONFIG_HW_STATE
#include <linux/hw_ops.h>
#endif

int pmic_set_chg_current(unsigned short);
extern int bind_flag;
#define PB_LOG
#ifdef PB_LOG
	#define LOG(x, args...)		printk(x, ##args)
#else
	#define LOG(x, args...)		NULL
#endif
...
volatile static int cable_state = CABLE_NEVER_INSERT;
...
********************************************************************/
static ssize_t
show_DpDn(struct device *dev, struct device_attribute *attr, char *buf);
static DEVICE_ATTR(dpdn, 0644, show_DpDn, NULL);
...
static int USB_ADAPTER_STATUS = 0;

#if CONFIG_HW_STATE
static int get_DpDn_status(void)
{
	if (!CABLE_HAD_INSERT)
		return -1;

	return USB_ADAPTER_STATUS;
}
#endif
...
#if (defined CONFIG_ARCH_MX35 || defined CONFIG_ARCH_MX25 || defined CONFIG_ARCH_MX5)
/*
...
static irqreturn_t fsl_udc_irq(int irq, void *_udc)
{
	...
	int tmp = 0;
	struct fsl_usb2_platform_data *pdata = udc->pdata;
	if(cable_state == CABLE_NEVER_INSERT)
	cable_state = CABLE_HAD_INSERT;
	if (pdata->irq_delay)
		return status;
	...
	if (irq_src & USB_STS_INT) {
		...
		/* completion of dtd */
		if ((tmp=fsl_readl(&dr_regs->endptcomplete))) {
	...
	if (tmp){
		USB_ADAPTER_STATUS = 0;
//		printk("USB plugin\n");
	}
	else{
		USB_ADAPTER_STATUS = 3;
//		printk("Adapter plugin\n");
	}
...
int usb_gadget_register_driver(struct usb_gadget_driver *driver)
{
	...
	if (udc_controller->transceiver) {
		...
		pmic_set_chg_current(0x2); //240mA
		LOG("Debug:bind udc driver to gadget driver ok, set usb charge current to 240mA.\n");
		bind_flag = 1;
...
out:
	if (retval) {
		pmic_set_chg_current(0x4); //400mA
		LOG("Debug:bind udc driver to gadget driver abnormal, set usb charge current to 400mA.\n");
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	udc_controller->driver = 0;
	pmic_set_chg_current(0x4); //400mA
	LOG("Debug:unbind gadget and unhook driver, set usb charge current to 400mA.\n");
	bind_flag = 0;
...
//	printk("%s:dr_regs->USBSTS=%x\n",__FUNCTION__,fsl_readl(&dr_regs->usbsts));
	if (pdata->have_sysif_regs)
		usb_sys_regs = (struct usb_sys_interface *)
				((u32)dr_regs + USB_DR_SYS_OFFSET);
#ifdef CONFIG_HW_STATE
	hw_set_getusb_DpDn(get_DpDn_status);
...
#if !(defined CONFIG_ARCH_MX35 || defined CONFIG_ARCH_MX25)
{
  ...
  if (device_create_file(udc_controller->gadget.dev.parent,
				&dev_attr_dpdn) < 0)
		printk(KERN_ERR "ARCOTG: Error creating device DpDn status file\n");
...
static int __exit fsl_udc_remove(struct platform_device *pdev)
{
	...
	device_remove_file(udc_controller->gadget.dev.parent, &dev_attr_dpdn);
	remove_proc_file();
...
static int fsl_udc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	dr_clk_gate(false);
 	reg_value = BITFVAL(USBEN, 0);
 	mask = BITFMASK(USBEN);
 	CHECK_ERROR(pmic_write_reg(32, reg_value, mask));	
	return ret;
}
