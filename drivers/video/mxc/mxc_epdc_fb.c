 ...
 * Copyright (C) 2011-2012 Foxconn TMSBG Co., Ltd. All Rights Reserved.
 *
...
//#define V220_E156_WAVEFORM_SIZE (1024*1024)
			       ...
			       bool use_test_mode, u32 np_val)
{
	...
	if(wf_type == 0x2D && waveform_mode == 3)
		waveform_mode = 2;

	if(wf_type == 0x15 || wf_type == 0x1D){
			if( waveform_mode == 6)
				waveform_mode = 1;
			else if( waveform_mode == 7){
					waveform_mode = 2;
				}
	}
	//printk(KERN_DEBUG "============waveform_mode=0x%0x\n", waveform_mode);
	if (use_test_mode) {
		reg_val |=
...
void epdc_init_settings(struct mxc_epdc_fb_data *fb_data)
{
	...
	/* EPDC_TEMP - Use environment temperature to get index */
	epdc_set_temp(mxc_epdc_fb_get_temp_index(fb_data, g_eink_temp));
...
static int mxc_epdc_fb_get_temp_index(struct mxc_epdc_fb_data *fb_data, char temp)
{
	int i;
	int index = -1;
	...
	/* if temperaturen < 0'c then use index : 0 */
	if (temp < 0)
		return 0; 
	...
	/* if temperature > max temp_range_bounds then use the max index */
	if (temp >= fb_data->temp_range_bounds[fb_data->trt_entries - 1])
		index = fb_data->trt_entries - 2;  
				...
				struct update_desc_list *update_to_merge)
{
	...
	if (a->waveform_mode != b->waveform_mode) {
		if (a->waveform_mode == g_fb_data->wv_modes.mode_a2 ||
			b->waveform_mode == g_fb_data->wv_modes.mode_a2) {
			//A2 should not be merge unless both have same waveform mode
			return MERGE_FAIL;
		}
...
static void epdc_submit_work_func(struct work_struct *work)
{
	...
	if (upd_data_list->update_desc->upd_data.temp != TEMP_USE_AMBIENT) {
		...
		dev_dbg(fb_data->dev, "Use User define temperature[%d\'C] \
				value for waveform temp index!\n", upd_data_list->update_desc->upd_data.temp);
		epdc_set_temp(temp_index);
	} else {
		fb_data->temp_index = mxc_epdc_fb_get_temp_index(fb_data, g_eink_temp);
		dev_dbg(fb_data->dev, "temperature is %d\'C, temp_index is %d\n", g_eink_temp, fb_data->temp_index);
	...
	dev_dbg(fb_data->dev, "Send_Update :\n\
...
#ifdef DEBUG
	pr_info("Before epdc_submit_update :\n");
	printk(KERN_DEBUG "EPDC_STATUS 0x%x\n", __raw_readl(EPDC_STATUS));	
	printk(KERN_DEBUG "EPDC_DEBUG_LUT0 0x%x\n", __raw_readl(EPDC_DEBUG_LUT0));
#endif
	epdc_submit_update(upd_data_list->lut_num,
...
#ifdef DEBUG
	pr_info("After epdc_submit_update :\n");
	printk(KERN_DEBUG "EPDC_STATUS 0x%x\n", __raw_readl(EPDC_STATUS));	
	printk(KERN_DEBUG "EPDC_DEBUG_LUT0 0x%x\n", __raw_readl(EPDC_DEBUG_LUT0));
#endif
	spin_unlock_irqrestore(&fb_data->queue_lock, flags);
...
#ifdef DEBUG	
	pr_info("******* dump_epdc_reg() start *******\n");
	dump_epdc_reg();
	pr_info("******* dump_epdc_reg() end *******\n");
#endif
	return 0;
}
						     ...
						     void *context)
{
	...
	if (fw == NULL) {
		...
		ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
			fb_data->fw_str, fb_data->dev, GFP_KERNEL, fb_data,
	...
	pr_info("Load Default waveform from Kernel...\n");
...
#ifdef KERNEL_LOGO
	/* Logo image data copy to framebufferr */
//	pr_info("bbp = %d\n", screeninfo->bits_per_pixel);
	upd_buf_ptr = (u16 *)fb_data->info.screen_base;
	memcpy(upd_buf_ptr, fsl_logo, xres * yres * 2);
#endif
...
static int mxc_epdc_fb_init_hw(struct fb_info *info)
{
	...
	wv_file = (struct mxcfb_waveform_data_file *)waveform_mem;

	waveform_size = wv_file->wdh.wi1;
	pr_info("Load waveform size %d Bytes\n", waveform_size);
	
	wf_type = (((wv_file->wdh.wi4) >> 24) & 0xff);
	pr_info("wf type is 0x%0x\n",wf_type);

	wf_mode = (wv_file->wdh.wi4 & 0xff);
	pr_info("wf version is 0x%0x\n", wf_mode);
	...
	/* Get offset and size for waveform data */
	wv_data_offs = sizeof(wv_file->wdh) + fb_data->trt_entries + 1;
	fb_data->waveform_buffer_size = waveform_size - wv_data_offs;
	...
	memcpy(fb_data->waveform_buffer_virt, (u8 *)(waveform_mem) + wv_data_offs,
		fb_data->waveform_buffer_size);
	
	iounmap(waveform_mem);

	free_bootmem(WAVEFORM_BASE, WAVEFORM_RESERVED_SIZE);

	pr_info("Load Waveform from moviNand Completed...\n");
...
static int mxc_epdc_fb_init_hw(struct fb_info *info)
{
	...
	pr_info("Load Waveform Firmware :%s\n", fb_data->fw_str);
...
int __devinit mxc_epdc_fb_probe(struct platform_device *pdev)
{
	...
	//pr_info("iMX508 EPDC driver for DVT board...\n");
	...
	/* Set default vmode */
	fb_data->cur_mode = &fb_data->pdata->epdc_mode[1]; //E60_V220
	...
	fb_data->wv_modes.mode_gl16 = 2;
	fb_data->wv_modes.mode_a2in = 6;
	fb_data->wv_modes.mode_a2out = 7;
