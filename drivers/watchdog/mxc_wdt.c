...
struct timer_list mxc_wdt_timer;
int hw_set_wdtctl(int (*set_wdt)(int));
static void poweroff_service_wdg(unsigned long data);
static void mxc_wdt_handle_timer(unsigned long data);
static int ep7_mxc_ctl_wdt(int new_time);
...
static int ep7_mxc_ctl_wdt(int new_time)
{
        printk("MXC WatchDog Set time to %ds\n", new_time);
	if(new_time == 0)
	{
	...
	if (test_and_set_bit(1, (unsigned long *)&mxc_wdt_users))
		return -EBUSY;
	if ((new_time < TIMER_MARGIN_MIN) ||
            (new_time > TIMER_MARGIN_MAX)) {
                printk("MXC watchdog error. wrong timer_margin %d\n",
                        timer_margin);
                printk("    Range: %d to %d seconds\n", TIMER_MARGIN_MIN,
                        TIMER_MARGIN_MAX);
                return -EINVAL;
        }
	mxc_wdt_adjust_timeout(new_time);
        mxc_wdt_disable(wdt_base_reg);
        mxc_wdt_set_timeout(wdt_base_reg);
       	mxc_wdt_enable(wdt_base_reg);
        mxc_wdt_ping(wdt_base_reg);
	mxc_wdt_timer.expires = jiffies + (new_time-2)*HZ;
	add_timer(&mxc_wdt_timer);
	}
	return 0;
}
...
static int __init mxc_wdt_probe(struct platform_device *pdev)
{
	...
	pr_info("MXC Watchdog # %d Timer: initial timeout %d sec\n", dev_num,
		timer_margin);
	init_timer(&mxc_wdt_timer);
        mxc_wdt_timer.function = mxc_wdt_handle_timer;
  init_timer(&poweroff_timer);
        poweroff_timer.expires = jiffies + (1 * HZ);
	      poweroff_timer.data = (unsigned long) pdev;
        poweroff_timer.function = poweroff_service_wdg;

	mxc_wdt_config(wdt_base_reg);
        mxc_wdt_set_timeout(wdt_base_reg);
        mxc_wdt_disable(wdt_base_reg);
        //mxc_wdt_ping(wdt_base_reg);
	hw_set_wdtctl(ep7_mxc_ctl_wdt);
...
{
	mxc_wdt_ping(wdt_base_reg);
	mod_timer(&poweroff_timer, jiffies + (30 * HZ));
	//printk("-------------%s-------------\n", __func__);
}
