...
#define SD1_WP	0	/*GPIO_4_19 */
#define SD1_CD	(0*32 + 27)	/*GPIO_1_27 */
#define SD1_EN	(3*32 + 2)	/*GPIO_4_2 */
#define SD1_DAT0	(4*32 + 2) /*GPIO_5_2*/
#define SD1_DAT1	(4*32 + 3) /*GPIO_5_3*/
#define SD1_DAT2	(4*32 + 4) /*GPIO_5_4*/
#define SD1_DAT3	(4*32 + 5) /*GPIO_5_5*/
#define SD1_CMD		(4*32 + 1) /*GPIO_5_1*/
#define SD2_WP	(4*32 + 16)	/*GPIO_5_16 */
#define SD2_CD	(4*32 + 17) /*GPIO_5_17 */
#define HP_DETECT	(3*32 + 15)	/*GPIO_4_15 */
#define HP_EN (5*32 + 10)	/*GPIO_6_10 */
#define PWR_INT		(3*32 + 14)	/*GPIO_4_14 */
...
#define LBATT_INT         (3*32 + 7)         
#define TOUCH_INT		(5*32 + 14)     /* GPIO_6_14 */
#define TOUCH_RST		(5*32 + 15)	/* GPIO_6_15 */
#define TOUCH_POWER		(4*32 + 17)	/* GPIO_5_17*/
#define CHRG_Indicator (5*32 + 13) /* GPIO_6_13*/   //add by sunny for led

#define IOC_RST (3*32 + 0) /*GPIO_4_0*/ 
#define IOC_EN (5*32 + 9) /*GPIO_6_9*/
//#define MOUSE_KEY (5*32 + 24) /*GPIO_6_24*/ 

#define WIFI_EN (3*32 + 5) /*GPIO_4_5*/
#define WIFI_RST (4*32 + 28) /*GPIO_5_28*/
...
static int fp9928_regulator_init(struct fp9928_data *fp9928_dev);
//static int max17135_regulator_init(struct max17135 *max17135);
static int battery_get_online(void);
static int battery_get_charger_online(void);
static int battery_get_charger_full(void);
...
static iomux_v3_cfg_t mx50_rdp[] = {
	...
	MX50_PAD_SD1_EN__GPIO_4_2,

	/* SD2 */
	//MX50_PAD_SD2_CD__GPIO_5_17,//touch power enable pin
	//MX50_PAD_SD2_WP__GPIO_5_16,//led enable pin
	...
	/* Audio */
	MX50_PAD_SSI_RXD__SSI_RXD, // SSI 
	MX50_PAD_SSI_TXD__SSI_TXD,
	MX50_PAD_SSI_TXC__SSI_TXC,
	MX50_PAD_SSI_TXFS__SSI_TXFS,
	MX50_PAD_ECSPI1_SS0__GPIO_4_15, /* HP_DET_B (headphone detect) */
	MX50_PAD_OWIRE__CCM_SSI_EXT1_CLK, /* MCLK */
	MX50_PAD_UART2_TXD__GPIO_6_10, /* SSM2602 HP EN */
...
/* add patch for cpu crash issue from freescale 20120507 */
...
static struct regulator_init_data fp9928_init_data[] = {
	{
		.constraints = {
			.name = "DISPLAY",
			.valid_ops_mask =  REGULATOR_CHANGE_STATUS,
...
//For E-ink waveform TFT timing required. Jerry
static struct fb_videomode e60_v220_mode = {
	.name = "E60_V220",
	.refresh = 85,
	.xres = 800,
...
static struct mxc_epdc_fb_mode panel_modes[] = {
...
#endif
	{
		...
		10,	/* sdoez_width */
		20,	/* sdoez_delay */
		403,/* gdclk_hp_offs */  //For E-ink waveform LGON timing required. Jerry
		20,	/* gdsp_offs */
		0,	/* gdoe_offs */
		9,	/* gdclk_offs */
		1,	/* num_ce */
...
static struct platform_device fp9928_sensor_device = {
	.name = "fp9928_sensor",
	.id = 0,
};
...
static struct fp9928_platform_data fp9928_pdata __initdata = {
	.gpio_pmic_vcom_ctrl = EPDC_PWRCOM,
	.gpio_pmic_wakeup 	 = EPDC_PWRSTAT,
	.regulator_init 	 = fp9928_init_data,
	.init				 = fp9928_regulator_init,
};

static int fp9928_regulator_init(struct fp9928_data *fp9928_dev)
{
	struct fp9928_platform_data *pdata = &fp9928_pdata;
	int i, ret;

	fp9928_dev->max_wait = 0;  //no delay

	fp9928_dev->gpio_pmic_vcom_ctrl = pdata->gpio_pmic_vcom_ctrl;
	fp9928_dev->gpio_pmic_wakeup = pdata->gpio_pmic_wakeup;

	fp9928_dev->vcom_setup = false;
	fp9928_dev->init_done = false;

	for (i = 0; i < FP9928_NUM_REGULATORS; i++) {
		ret = fp9928_register_regulator(fp9928_dev, i,
			&pdata->regulator_init[i]);
		if (ret != 0) {
			printk(KERN_ERR"fp9928 regulator init failed: %d\n",
				ret);
			return ret;
		}
	}
...
static int battery_get_charger_online(void){
	int sense;
	pmic_read_reg(REG_INT_SENSE0, &sense, 0xffffffff);
	return (sense & (1 << 6));
}
...
static struct max17040_platform_data max17040_pdata = {
       .battery_online = battery_get_online,
       .charger_online = battery_get_charger_online,
       .charger_full = battery_get_charger_full,
};
...
static struct i2c_board_info mxc_i2c0_board_info[] __initdata = {
	 ...
	 },*/
	{
		 ...
		 .platform_data = &max17040_pdata,
		  .irq = gpio_to_irq(LBATT_INT),
	},
	{
		I2C_BOARD_INFO("fp9928_pmic", 0x48),
		.platform_data = &fp9928_pdata,
	},
	/* 
	{
	 .type = "mma8450",
...
static unsigned int sdhc_get_card_det_status(struct device *dev)
{
	...
	if (to_platform_device(dev)->id == 0) {
		...
		//printk("+++++++++++++++++++++++++++++++++++ret is:%d\n", ret);
		if(ret == 0) {
			hw_set_sd(1); 			//SD card is in, set for the IO driver.  
			
			/*open the power while SD card in.*/
			gpio_request(SD1_EN, "sdhc1-en");
			gpio_direction_output(SD1_EN, 0);			
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_CMD__SD1_CMD);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_D0__SD1_D0);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_D1__SD1_D1);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_D2__SD1_D2);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_D3__SD1_D3);
			gpio_free(SD1_EN);

		}
		else { 
			hw_set_sd(0);			//SD card is not in.
		
			/*shut the power while SD card is not in.*/	
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_EN__GPIO_4_2);
			gpio_request(SD1_EN, "sdhc1-en");
			gpio_direction_output(SD1_EN, 1);			
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_CMD__GPIO_5_1);
			gpio_request(SD1_CMD, "sd1_cmd");
			gpio_direction_output(SD1_CMD, 0);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_D0__GPIO_5_2);
			gpio_request(SD1_DAT0, "sd1_dat0");
			gpio_direction_output(SD1_DAT0, 0);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_D1__GPIO_5_3);
			gpio_request(SD1_DAT1, "sd1_dat1");
			gpio_direction_output(SD1_DAT1, 0);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_D2__GPIO_5_4);
			gpio_request(SD1_DAT2, "sd1_dat2");
			gpio_direction_output(SD1_DAT2, 0);
			mxc_iomux_v3_setup_pad(MX50_PAD_SD1_D3__GPIO_5_5);
			gpio_request(SD1_DAT3, "sd1_dat3");
			gpio_direction_output(SD1_DAT3, 0);
			gpio_free(SD1_EN);
			gpio_free(SD1_CMD);
			gpio_free(SD1_DAT0);
			gpio_free(SD1_DAT1);
...
/* add for EP7 using ssm2602 codec */
static struct clk *ssi_ext1_clk;
static int mxc_ssm2602_clock_get(void)
{
	unsigned long freq;
	ssi_ext1_clk = clk_get(NULL, "ssi_ext1_clk");
	freq = clk_get_rate(ssi_ext1_clk);
	printk(KERN_INFO "ssi_ext1_clk=%lu\n", freq);
	return 0;
}

static int mxc_ssm2602_clock_enable(int enable)
{
	ssi_ext1_clk = clk_get(NULL, "ssi_ext1_clk");
	if(enable)
...
static int mxc_ssm2602_hp_enable(int enable) 
{
	gpio_set_value(HP_EN, enable);
	return 0;
}
...
static struct mxc_audio_platform_data ssm2602_data = {
	.ssi_num = 1,
	.src_port = 2,
	.ext_port = 3,
	.hp_irq = gpio_to_irq(HP_DETECT),
	.hp_status = headphone_det_status,
	.hp_enable = mxc_ssm2602_hp_enable,
	.clock_enable = mxc_ssm2602_clock_enable,
	.sysclk = 12000000,
};

static struct platform_device mxc_ssm2602_device = {
...
static void __init mx50_rdp_io_init(void)
{
	...
	gpio_request(HP_EN, "hp-en");
	gpio_direction_output(HP_EN, 0);
	...
	*/
	...
	/* EP7 Eink PMIC WAKEUP*/
	gpio_request(EPDC_PWRSTAT, "epdc-pmic-wake");
	gpio_direction_output(EPDC_PWRSTAT, 0);
	
	/* EP7 Eink PMIC VCOM*/
	gpio_request(EPDC_PWRCOM, "epdc-vcom");
	gpio_direction_output(EPDC_PWRCOM, 0);
	
	gpio_request(ELCDIF_PWR_ON, "elcdif-power-on");
	gpio_direction_output(ELCDIF_PWR_ON, 1);

/*************add by hugo for touch driver***********/
	...
	gpio_request(TOUCH_POWER, "touch_power");
	printk("**********touch power on***************\n");
	gpio_direction_output(TOUCH_POWER,1);
	gpio_direction_output(TOUCH_RST, 0);
	mdelay(5);
	gpio_direction_output(TOUCH_RST, 1);

	/*************add by sunny for low battery***********/
	gpio_request(LBATT_INT, "low battery");
	gpio_direction_input(LBATT_INT);
	...
	*/
	...
	/* WIFI MODULE POWER and RESET */
	gpio_request(WIFI_EN, "wifi power");
	...
	/* GPIO FOR LED*/
	gpio_request(CHRG_Indicator, "led on");
...
static void __init mxc_board_init(void)
{
	...
	mxc_register_device(&mxci2c_devices[0], &mxci2c_data[0]);
	mxc_register_device(&mxci2c_devices[1], &mxci2c_data[1]);
	mxc_register_device(&mxci2c_devices[2], &mxci2c_data[2]);
...
//add for nand 
//	mxc_register_device(&mxc_nandv2_device,);

	//mxc_register_device(&max17135_sensor_device, NULL);
	mxc_register_device(&fp9928_sensor_device, NULL); 
	//mxc_register_device(&usb_adapter_device, NULL); 
...
/*
	...
	*/
	...
	mxc_register_device(&mxc_ssm2602_device, &ssm2602_data);
	mxc_ssm2602_clock_get(); //add for output audio clk
	mxc_ssm2602_clock_enable(1);// enable audio clk
//	mxc_register_device(&mxc_sgtl5000_device, &sgtl5000_data);
//	mxc_register_device(&gpmi_nfc_device, &gpmi_nfc_platform_data);
	mx5_usb_dr_init();
	mx5_usbh1_init();
	mxc_register_device(&mxc_perfmon, &mxc_perfmon_data);
	mxc_register_device(&gpiokeys_device, &gpiokeys_platdata);

	hw_set_device_reboot(arm_pm_restart);
}
