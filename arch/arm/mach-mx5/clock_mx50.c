...
//Add function for pdf_clock[0~7] get rate. Jerry
static unsigned long pfd_get_rate(struct clk *clk)
{
	...
	val = __raw_readl(apll_base + (int)clk->enable_reg);

	val = (val >> clk->enable_shift) & MXC_CCM_EPDC_AXI_DIV_MASK;
...
static int _clk_epdc_pix_set_rate(struct clk *clk, unsigned long rate)
{
	...
	new_div--;  //For 26MHz pixclock adjustment, Jerry
...
int __init mx50_clocks_init(unsigned long ckil, unsigned long osc, unsigned long ckih1)
{
	...
	//clk_enable(&pfd5_clk);
	clk_set_rate(&pfd5_clk, 261818181);  //For 26MHz pixclock adjustment, Jerry

	clk_enable(&epdc_pix_clk);
	clk_set_parent(&epdc_pix_clk, &pfd5_clk);
	clk_disable(&epdc_pix_clk);
