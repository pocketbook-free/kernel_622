 ...
 */
static void ssm2602_clock_gating(struct snd_soc_codec *codec, int enable)
{
	struct ssm2602_priv *ssm2602 = snd_soc_codec_get_drvdata(codec);

	if (ssm2602->clock_enable == NULL)
		return;

	if (enable == 0) {
		if (!ssm2602->need_clk_for_access &&
		    !ssm2602->need_clk_for_bias)
			schedule_delayed_work(&codec->delayed_work,
					      msecs_to_jiffies(300));
	} else {
		if (!ssm2602->clock_on) {
			ssm2602->clock_enable(1);
			ssm2602->clock_on = 1;
		}
	}
}
	...
	unsigned int value)
{
	...
	data[1] = value & 0x00ff;
	//ssm2602->need_clk_for_access = 1;
 // ssm2602_clock_gating(codec, 1);
	ssm2602_write_reg_cache(codec, reg, value);
	if (codec->hw_write(codec->control_data, data, 2) == 2) { 
	// ssm2602->need_clk_for_access = 0;
  // ssm2602_clock_gating(codec, 0);
   return 0;}
	else 
	 return -EIO; 
...
SOC_DOUBLE_R("Capture Volume", SSM2602_LINVOL, SSM2602_RINVOL, 0, 31, 0), //should be 63?
...
static int ssm2602_mute(struct snd_soc_dai *dai, int mute)
{
	...
	//printk("%s:mute=%d, open=%d\n", __func__, mute, open);
	if (mute){
		if(0 == open)
		{
			gpio_set_value(HP_EN, 0);
			ssm2602_write(codec, SSM2602_APDIGI, mute_reg | APDIGI_ENABLE_DAC_MUTE);
		}
			open = 1;
	}
	else{
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		/* keep clock on in prepare status */
		//ssm2602->need_clk_for_bias = 1;
		//ssm2602_clock_gating(codec, 1);
		reg = ssm2602_read_reg_cache(codec, SSM2602_PWR);
		reg &= ~(PWR_DAC_PDN | PWR_ADC_PDN);
		ssm2602_write(codec, SSM2602_PWR, reg);
		reg = ssm2602_read_reg_cache(codec, SSM2602_PWR);
		reg &= ~(PWR_CLK_OUT_PDN | PWR_OSC_PDN | PWR_LINE_IN_PDN | PWR_MIC_PDN | PWR_OUT_PDN);
		ssm2602_write(codec, SSM2602_PWR, reg);
		reg = ssm2602_read_reg_cache(codec, SSM2602_PWR);
    reg |= PWR_POWER_OFF;
		ssm2602_write(codec, SSM2602_PWR, reg);
	...
	case SND_SOC_BIAS_STANDBY:
		...
		ssm2602_write(codec, SSM2602_PWR, reg);
		reg = ssm2602_read_reg_cache(codec, SSM2602_PWR);
		reg |= (~PWR_POWER_OFF | PWR_CLK_OUT_PDN | PWR_OSC_PDN | PWR_LINE_IN_PDN | PWR_MIC_PDN | PWR_OUT_PDN);
		ssm2602_write(codec, SSM2602_PWR, reg);
		//printk(KERN_INFO "%s REG : 0x%x\n", __func__, reg);
		//ssm2602->need_clk_for_bias = 0;
		//ssm2602_clock_gating(codec, 0);
		break;
 ...
 */
static void ssm2602_work(struct work_struct *work)
{
...
static int ssm2602_init(struct snd_soc_device *socdev)
{
	...
	/*select DAC as default output */
	ssm2602_write(codec, SSM2602_APANA, APANA_SELECT_DAC |
			APANA_ENABLE_MIC_BOOST);
  /* power up whole chip and elements*/
...
static int ssm2602_probe(struct platform_device *pdev)
{
	...
	//	ssm2602->clock_enable = setup->clock_enable;
	//	ssm2602->need_clk_for_bias = 1;
	//	INIT_DELAYED_WORK(&codec->delayed_work, ssm2602_work);
	//}
